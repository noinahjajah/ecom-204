/**
 * payment.js — ฟังก์ชันช่วยเหลือการชำระเงิน
 * Luhn check, format card number, detect card brand, etc.
 */

/**
 * ตรวจสอบความถูกต้องของหมายเลขบัตรด้วย Luhn Algorithm
 * @param {string} cardNumber - หมายเลขบัตร (มีหรือไม่มีช่องว่างก็ได้)
 * @returns {boolean}
 */
export function luhnCheck(cardNumber) {
  const clean = String(cardNumber).replace(/\s/g, "").replace(/-/g, "");
