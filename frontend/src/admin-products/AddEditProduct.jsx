  ...
  return (
    <div>
      ...
      <div className="admin-repeatable-row">
        <input className="admin-input" value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); add(); } }} placeholder="https://... (URL รูปเพิ่มเติม)" />
      ...
      <div className="admin-repeatable" style={{ marginBottom: 16 }}>
        ...
        {options.map((o, idx) => (
          <div className="admin-repeatable-row" key={idx}>
            ...
            <input className="admin-input" value={(o.values || []).join(", ")} onChange={(e) => updateOption(idx, { values: e.target.value.split(",").map((x) => x.trim()).filter(Boolean) })} placeholder="ค่าตัวเลือก คั่นด้วย comma เช่น 30ml, 50ml" />
        ...
        {rows.map((v, idx) => (
          <div key={idx} className="admin-panel" style={{ padding: 12 }}>
            ...
            <Field label={`ตัวเลือก (${options.map((o) => o.name).filter(Boolean).join(", ") || "-"})`}>
              <input className="admin-input" value={Object.entries(v.options || {}).map(([k, val]) => `${k}:${val}`).join(", ")} onChange={(e) => {
                const map = {};
...
export default function AddEditProduct() {
  ...
  const backToTable = () => { window.location.href = "/admin/products"; };
      ...
      <div className="admin-page admin-form-shell">
        <div className="admin-header-row">
          ...
          <div className="admin-header-actions">
            <button className="admin-btn" type="button" disabled={saving} onClick={() => onSave({ draft: true })}>บันทึกเป็น Draft</button>
            <button className="admin-btn admin-btn-gold" type="button" disabled={saving} onClick={() => onSave({ draft: false })}>{saving ? "กำลังบันทึก..." : "บันทึก"}</button>
        ...
        <div className="admin-form-grid">
          <Field label="ชื่อสินค้า" error={errors.name}>
            <input className={"admin-input" + (errors.name ? " admin-field-error" : "")} value={product.name} onChange={(e) => set({ name: e.target.value, seo: { ...(product.seo || {}), urlSlug: product.seo?.urlSlug ? product.seo.urlSlug : slugify(e.target.value) } })} />
          </Field>
        ...
        <div className="admin-form-grid is-3">
          ...
          <Field label="ราคาโปร (ถ้ามี)">
            <input className="admin-input" type="number" value={product.promoPrice ?? ""} onChange={(e) => set({ promoPrice: e.target.value === "" ? null : Number(e.target.value) })} placeholder="(optional)" />
          </Field>
        ...
        <VariantsEditor variantOptions={product.variantOptions} variants={product.variants} onOptionsChange={(variantOptions) => set({ variantOptions })} onVariantsChange={(variants) => set({ variants })} />
        ...
        <div className="admin-form-grid is-4">
          ...
          <Field label="ค่าจัดส่ง (บาท)">
            <input className="admin-input" type="number" value={product.shipping?.shippingFee ?? 0} onChange={(e) => setShipping({ shippingFee: Number(e.target.value) })} disabled={!!product.shipping?.freeShipping} />
          </Field>
